...
class CertificationDescriptions: NSObject {
    ...
    public func getCertDescription(functionName: String)->Array<String>{
        ...
        if(functionName == "crueltyFree" ){
            return ["Cruelty Free", notYetDefined]
        }
        if(functionName == "ethicalTrading" ){
            return ["Ethical Trading", notYetDefined]
        }
        if(functionName == "fairmined" ){
            return ["Fair Mined", notYetDefined]
        }
        ...
        if(functionName == "vegetarian" ){
            return ["Vegetarian", notYetDefined]
        }
        if(functionName == "workplaceConditions" ){
            return workplaceCondition
        }
        else {
            return ["Name undefined", notYetDefined]
        }
    }
    
    public let notYetDefined = "This description has not yet been defined"
    
    public let fairTrade = ["Fair Trade",
    ...
    public let betterCotton = ["Better Cotton",
                               "The Better Cotton Initiative exists to make global cotton production better for the people who produce it, better for the environment it grows in, and better for the sector’s future. BCI connects people and organisations from across the cotton sector, from field to store, to promote measurable and continuing improvements for the environment, farming communities and the economies of cotton-producing areas."]
    
    public let blueSign = ["BlueSign",
                           "The bluesign® SYSTEM is focused on people, the environment and resources. Through its comprehensive approach based on Input Stream Management, the bluesign® SYSTEM reduces the textile industry’s impacts on people and the environment, ensures responsible use of resources and guarantees the highest degree of consumer protection. The strictest criteria and analysis for on-site system implementation motivate companies along the entire value chain to improve their sustainability performance with BLUESIGN."]
    
    public let canopyStyle = ["Canopy",
    ...
    public let globalRecylce = ["Global Recyclable",
    ...
    public let leatherWorkingGroup = ["Leather Working Group",
                                      "The Leather Working Group (LWG) is a not-for-profit membership organisation for stakeholders across the leather supply chain. Since 2005, we have developed audit protocols designed to certify leather manufacturers according to their environmental compliance and performance capabilities."]
    
    public let nordicSwan = ["Nordic Swan",
    ...
    public let responsibleDown = ["Responsible Down",
    ...
    public let responsibleWool = ["Responsible Wool",
    ...
    public let soilOrganic = ["Soil Association",
    ...
    public let workplaceCondition = ["Workplace Conditions",
